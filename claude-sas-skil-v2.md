...
> **⚠️ Hard constraint:** Every fact written into an output MD file must be traceable
> to a line in the actual SAS source code. If it cannot be, trigger the
> **Ambiguity Protocol** (see below) before proceeding. Never assume, infer from naming
> conventions alone, or invent logic to fill a gap.
...
> If any element cannot be confirmed from the source code, **stop and trigger the
> Ambiguity Protocol** before continuing to the next element. Do not make a best-guess
> and move on.
...
- [ ] **Guardrails** — No cell contains invented column names, assumed logic, or placeholder text without a `[UNRESOLVED]` tag.
- [ ] **Validation** — All cross-section checks passed (see Validation Logic below).
- [ ] **Ambiguities** — All triggered ambiguities are resolved or tagged `[UNRESOLVED]` in Section 9.

---

## Core Guardrails

These are absolute constraints. Any violation invalidates the output MD file.

| # | NEVER DO THIS |
|---|---|
| G1 | Invent a column name not present verbatim in the SAS source code. |
| G2 | List a table in Section 2 that does not appear in FROM, SET, MERGE, or JOIN in the code. |
| G3 | Document a join key unless it is explicitly written in an ON clause. |
| G4 | Write "Direct pass-through" unless the column name and value are identical from source to output with zero expression applied. |
| G5 | Assume a macro's effect without reading its `%MACRO` definition. If the definition is absent, trigger the Ambiguity Protocol. |
| G6 | Assert that script A depends on script B without an explicit reference in the code. |
| G7 | Skip an intermediate TRN step in a lineage chain. Every hop must be documented. |
| G8 | Leave a section blank or write "TBD", "N/A", or "unclear" without appending `— reason: [explanation]`. |
| G9 | Begin parsing the next script while an unresolved ambiguity for the current script is still open. |
| G10 | Infer transformation logic or business rules from a column name, table name, or naming convention alone. |
| G11 | Apply a rule learned from one script to another without verifying it in that script's code. |
| G12 | Write a lineage path that ends at a TRN script — chains must terminate at an EXT reference or a hard-coded literal. |
...
- A column's source table or source column cannot be determined from the code.
- A macro's effect on output attributes is unknown (definition not found).
- A join key, filter condition, or aggregation grouping is implied but not written.
- Two interpretations of a code block are equally plausible.
- A referenced table or script is absent from the folder and its role is unclear.
- A `%LET` / macro variable resolves to multiple possible values.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Script   : [script_name]
Section  : [e.g., Section 3 — Attribute Inventory]
Issue    : [plain-English description of what is unclear]
Evidence : [quote the relevant line(s) of SAS code]

Options:
  A) [Most likely interpretation, with brief rationale]
  B) [Alternative interpretation, with brief rationale]
  C) Skip this item — mark it [UNRESOLVED] in the output MD
  D) I'll provide more context (paste code, schema, or data dictionary)

Reply with A, B, C, or D.
```

**Rules:**
- Ask about one ambiguity at a time. Never batch multiple unrelated questions.
- If the user selects D, re-read the provided context and attempt resolution before re-asking.
- If the user selects C, write `[UNRESOLVED — <issue summary>]` in the affected cell and note it in Section 9.
- Record every ambiguity and its resolution in **Section 9 (Agent Notes)** of the output MD.
- Do not proceed to the next script until the current script's ambiguities are resolved or tagged.

---

## Validation Logic

Run these checks **after drafting each section** and **before writing the file**.
If any check fails: do not write the file — report the failure and trigger the Ambiguity Protocol.

### Section 2
- [ ] Every table in Section 2 appears verbatim in FROM / SET / MERGE / JOIN in the script.
- [ ] No table is listed that cannot be found in the script code.

### Section 3
- [ ] Row count ≥ number of SELECT columns or DATA step output assignments in the script.
- [ ] Every Source Table in Section 3 is present in Section 2.
- [ ] No Source Column is listed that cannot be traced to an expression in the code.
- [ ] "Transformation Logic" is non-empty for every CASE, calculation, or function result.
- [ ] "Direct pass-through" is used only where column name and value are provably identical.

### Section 4
- [ ] Every PROC SQL block and DATA step in the code has a plain-English paragraph.
- [ ] No paragraph describes logic absent from the script.

### Section 5
- [ ] Every WHERE, IF/THEN/ELSE, and CASE WHEN in the script appears in Section 5.
- [ ] Each condition has a business-meaning statement — not just a code restatement.

### Section 6
- [ ] Every lineage path starts at this script and ends at an EXT reference or a literal.
- [ ] No intermediate TRN step is skipped.
- [ ] If Section 6 is omitted, no input to this script is from another TRN script.

### Cross-Section Consistency
- [ ] Attributes in Section 6 are a subset of attributes in Section 3.
- [ ] Macros in Section 7 appear in the script source code.
- [ ] Scripts in Section 8 are referenced in the code and absent from the folder.
- [ ] `[UNRESOLVED]` tags in any section have a corresponding entry in Section 9.

---

## Self-Improvement Loop

After completing **all** MD files for a batch run, perform the following steps.

### 1. Session Debrief (internal)
Review the run and identify:
- Every ambiguity triggered and how it was resolved.
- Edge cases not covered by the existing rules.
- SAS constructs encountered that the Parsing Guide does not address.
- Any guardrail or validation rule that was unclear or required judgment.

### 2. Generate a Skill Improvement Proposal
Write `knowledge_base/SKILL_proposal_v[N].md` (increment N from any existing proposals):

```markdown
# SKILL.md Improvement Proposal — v[N]
Generated after: [folder name or date]

## Ambiguity Log
| Script | Issue | Resolution | Suggested Rule Addition |
|---|---|---|---|

## New Edge Cases
| Situation Observed | Recommended Handling |
|---|---|

## Rule Clarifications
| Existing Rule | Ambiguity Observed | Proposed Wording |
|---|---|---|

## New Guardrails
| # | Proposed Rule | Why Needed |
|---|---|---|

## Parsing Guide Additions
(Any SAS construct encountered that current Parsing Guide does not cover)

## Validation Rule Additions
(Any cross-section inconsistency type not currently caught by Validation Logic)
```

### 3. Present to User
```
Self-Improvement Proposal Ready
─────────────────────────────────────────────────────
I encountered [N] ambiguities and [M] uncovered patterns this run.
Proposal saved to: knowledge_base/SKILL_proposal_v[N].md

What would you like to do?
  A) Review and apply approved changes to SKILL.md now
  B) Save the proposal for later — continue without changes
  C) Discard — this run was exploratory
```

### 4. Apply Approved Changes (if user selects A)
- Edit SKILL.md in place with the approved additions.
- Prepend a `## Changelog` block at the top of the file (below frontmatter):

```markdown
## Changelog
| Version | Date | Summary |
|---|---|---|
| v[N] | [date] | [one-line summary of what changed and why] |
```

- Do not remove or rewrite existing rules without explicit user approval.
- Additive changes only unless the user explicitly asks to replace a rule.
